...
## AI Workflow
- I primarily used Claude during implementation because it performed best at long-form code generation and maintaining consistency across HTML, CSS, and JavaScript files. For planning UI structure and interaction flow, I used ChatGPT to think through product direction, feature scope, and UX decisions like the mood flow, session persistence, and analytics layout. During execution, Claude handled most of the actual coding, especially for generating large cohesive blocks like the heatmap, charts, and timer logic. For polishing and reviewing, I switched back to ChatGPT because it was stronger at identifying architectural weaknesses, UX inconsistencies, and future scalability issues.
- One moment Claude clearly outperformed ChatGPT was generating the initial app structure quickly while keeping the visual style coherent across files. One moment I switched tools mid-task was when debugging timer persistence and refresh recovery. Claude generated working code, but ChatGPT was better at reasoning through state restoration edge cases and explaining why the original implementation could fail after reloads or paused sessions.
- The workflow became less about “asking one AI to do everything” and more about choosing the model best suited for planning, generation, debugging, or critique.

## Reflection

The biggest thing the workflow unlocked was speed. There is no way I could have designed and implemented this entire app in around 4 hours by myself while also keeping the UI consistent and adding features like charts, heatmaps, mood tracking, session logging, and timer persistence. Normally I would spend most of my time context switching between docs, Stack Overflow, debugging, and rewriting UI code over and over. Using Claude let me move much faster because I could stay focused on the product itself instead of getting stuck on implementation details for every feature. I was able to iterate on ideas quickly and see results immediately, which made the whole process feel much more fluid.

There were still multiple times where I had to step in and override Claude. The biggest example was around product feel and UX decisions. Claude could generate functional code, but it did not really understand the tone I wanted for the app. I wanted something calm, minimal, and low pressure instead of a typical productivity app full of bright colors and gamification. I also had to step in when the timer persistence logic broke after refreshes. Claude generated code that mostly worked, but I had to think through the actual state transitions and edge cases myself to understand what was happening. That part showed me that AI can generate solutions quickly, but it still does not truly understand the system it is building unless the developer does.

This project revealed that my judgment is currently more valuable than my raw coding speed. Before this, I thought technical skill mostly meant writing everything manually. What I realized is that the hard part is often deciding what should exist in the first place. I spent a lot of time evaluating generated code and deciding whether something actually matched the product I wanted. Sometimes Claude would generate technically correct solutions that still felt wrong for the app. I also realized I have gaps in areas like state management, browser timing behavior, persistence, and architecture decisions once projects grow larger. I could still build features, but I sometimes lacked confidence about whether the implementation would hold up long term. At the same time, I realized I am better at product thinking and design consistency than I expected. I was able to keep a clear vision for the app even while using AI heavily.

In my internship, I want to use AI as a collaborator instead of treating it like autocomplete. On day one, the first thing I would do is understand the codebase structure and identify repetitive or time consuming tasks where AI could accelerate implementation safely. I also want to get better at reviewing generated code critically instead of assuming it is correct because it runs. This project made me realize that the value of AI workflows comes from combining fast generation with strong human judgment. The people who benefit most from these tools are probably the ones who can clearly evaluate quality, spot problems early, and keep a consistent direction while moving quickly.